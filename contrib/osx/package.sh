...
rm -rf /tmp/pkpay-macos/image > /dev/null 2>&1
mkdir /tmp/pkpay-macos/image/
cp -r $1 /tmp/pkpay-macos/image/
...
${genisoimage} \
    ...
    -dir-mode 0755 \
    -apple \
    -o PKpay_uncompressed.dmg \
    /tmp/pkpay-macos/image || fail "Unable to create uncompressed dmg"

dmg dmg PKpay_uncompressed.dmg pkpay-$VERSION.dmg || fail "Unable to create compressed dmg"
rm PKpay_uncompressed.dmg

echo "Done."
md5sum pkpay-$VERSION.dmg
