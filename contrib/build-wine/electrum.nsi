...
;Variables
  ...
  !define PRODUCT_NAME "PKpay"
  !define PRODUCT_WEB_SITE "http://www.pkc.ink"
  !define PRODUCT_PUBLISHER "PKpay Technologies GmbH"
...
Section
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\pkpay-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\pkpay-${PRODUCT_VERSION}.exe" "" "$INSTDIR\pkpay-${PRODUCT_VERSION}.exe" 0
